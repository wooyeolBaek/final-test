...
from backend.model.face.utils.facial_analysis import FacialImageProcessing
...
def process_face_image(img, path):
    try:
        ...
        if len(best_bb) != 0:
            ...
            return Image.fromarray(face_img), box
        else:
            # print("No faces found for ", path)
            img_h, img_w, _ = img.shape
            box = [0, 0, img_w, img_h]
            return Image.fromarray(img), box
...
class customDataset(DatasetFolder):
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        sample, _ = process_face_image(sample, path)
        # print(type(sample))
...
class testDataset(Dataset):
    def __init__(self, path, transform=None, loader=default_loader):
        ...
        self.images = [os.path.join(path, image) for image in images]
    ...
    def __getitem__(self, idx):
        ...
        sample = self.loader(path)
        sample, box = process_face_image(sample, path)
        if self.transform:
            sample = self.transform(sample)
        return sample, box, path
